...
class TestAllSensors:
    ...
    sensor_logs = read_sensor_logs("appliance")
    ...
    def test_readings_have_valid_params(self, log):
        for reading in log:
            ...
            assert "operatingStatus" in param_types
    ...
    def test_at_least_one_reading(self, log):
        ...
        for reading in log:
            param_types = [param["name"] for param in reading["params"]]
            valid = valid or "operatingStatus" in param_types
            if valid:
                break
        assert valid
    ...
    @pytest.mark.parametrize("log", sensor_logs)
    def test_readings_operatingStatus_valid(self, log):
        for reading in log:
            for param in reading["params"]:
                if param["name"] == "operatingStatus":
                    assert param["value"] == '0' or param["value"] == '1'

    @pytest.mark.parametrize("log", sensor_logs)
    def test_readings_operatingStatus_event_based(self, log):
        past_value = None
        for reading in log:
            for param in reading["params"]:
                if param["name"] == "operatingStatus":
